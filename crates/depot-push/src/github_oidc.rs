...
pub async fn request_github_actions_oidc_token(
    ...
    let request_url = std::env::var("ACTIONS_ID_TOKEN_REQUEST_URL")
        .context("DEPOT_WRITE_TOKEN is unset and ACTIONS_ID_TOKEN_REQUEST_URL is unavailable")?;
    let request_token = std::env::var("ACTIONS_ID_TOKEN_REQUEST_TOKEN")
        .context("DEPOT_WRITE_TOKEN is unset and ACTIONS_ID_TOKEN_REQUEST_TOKEN is unavailable")?;
