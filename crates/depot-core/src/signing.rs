...
mod tests {
    ...
    fn public_key_texts_preserve_key_order() {
        let key1 = NamedSigningKey::parse(
            "depot.example.com-1:AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=",
        )
        .unwrap();
        let key2 = NamedSigningKey::parse(
            "depot.example.com-2:zFD7RJEU40VJzJvgT7h5xQwFm8FufXKH2CJPaKvh/xo=",
        )
        .unwrap();
        ...
        assert!(public_keys[0].starts_with("depot.example.com-1:"));
        assert!(public_keys[1].starts_with("depot.example.com-2:"));
    }
    ...
    fn narinfo_fingerprint_ignores_deriver_signatures_and_ca() {
        ...
        enriched.signatures = vec!["depot.example.com-1:abc".to_owned()];
    ...
    fn sign_returns_one_signature_per_key_in_input_order() {
        ...
        assert!(signatures[0].starts_with("depot.example.com-1:"));
        assert!(signatures[1].starts_with("depot.example.com-2:"));
    }
