...
pub async fn upsert_project_retention_policy(
    ...
    let rules = request
        .rules
        .into_iter()
        .map(|rule| depot_db::ProjectRetentionRuleRecord {
...
fn retention_policy_info(
    policy: depot_db::ProjectRetentionPolicyRecord,
...
fn upstream_infos(upstreams: Vec<depot_db::UpstreamCacheRecord>) -> Vec<UpstreamInfo> {
    upstreams
        .into_iter()
...
fn access_token_info_from_records(
    records: Vec<depot_db::AccessTokenRecord>,
) -> Vec<AccessTokenInfo> {
