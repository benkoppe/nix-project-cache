...
impl DepotClient {
    ...
    pub async fn register_paths(
        ...
        let response = self
            .request(Method::POST, url)
            .json(&depot_api::RegisterPathsRequest {
...
mod tests {
    ...
    struct MultipartPresignRequestRecord {
        ...
        part_number: i32,
        request: depot_api::PresignMultipartUploadPartRequest,
    }
    ...
    struct TestState {
        ...
        completed_multipart_uploads: SharedRecords<depot_api::CompleteMultipartUploadRequest>,
    ...
    async fn register_paths_handler(
        Json(request): Json<depot_api::RegisterPathsRequest>,
        ...
        (
            ...
            Json(RegisterPathsResponse {
                required_uploads: vec![depot_api::RequiredUpload {
                    store_path_hash: path.hash_str(),
                    object_path: path.url().to_owned(),
                    content_type: "application/octet-stream".to_owned(),
                    method: depot_api::UploadMethod::Proxy,
                }],
            }),
        )
    }
    ...
    async fn presign_multipart_part_handler(
        ...
        Json(request): Json<depot_api::PresignMultipartUploadPartRequest>,
    ) -> (
        StatusCode,
        Json<depot_api::PresignMultipartUploadPartResponse>,
    ) {
        state.auth_headers.lock().unwrap().push(
            headers
                .get(header::AUTHORIZATION)
    ...
    async fn complete_multipart_upload_handler(
        ...
        Json(request): Json<depot_api::CompleteMultipartUploadRequest>,
    ) -> StatusCode {
        state.auth_headers.lock().unwrap().push(
            headers
                .get(header::AUTHORIZATION)
    ...
    async fn finalize_handler() -> (StatusCode, Json<depot_api::FinalizeBuildResponse>) {
        (StatusCode::OK, Json(depot_api::FinalizeBuildResponse {}))
    }
